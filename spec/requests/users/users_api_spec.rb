...
RSpec.describe 'Users API Requests' do
	...
	describe 'GET /search', type: :request do
		let(:search_term) { 'thenoid' }
		let(:url) {"/api/v1/users/search"}

		subject { get url, params: { :term => search_term } }
		...
		context 'when user is signed in' do
			let!(:user1) { create :user, username: 'testthenoidtest', email: 'something@example.com' }
			let!(:user2) { create :user, username: 'something', email: 'thenoid@example.com' }
			let!(:user3) { create :user, username: 'somethingelse', email: 'somethingelse@example.com' }
			let(:expected_response) { [user1.display_user,user2.display_user] }
